...
def get_security_type(exchange_data, type = "Common Stock"):
    """Returns list of filtered securities avaliable:
    Common Stock, ETF, Fund, Preferred Stock"""
...
def returns_from_closes(folder, filename):
    """
    returns instantaneous returns froom selected securities
    """
...
#left off at part 5 Calculate Returns 2:11 timestamp

def get_corr(data):
    """
    returns correlation from securities
    """
...
def plot_closes(closes, relative=False):
    """
    plot absolute or relative closes for securities 6:00
    """
...
def get_return_data(*tickers, date = DEFAULT_DATE, adj_close=False, key):
    """
    saves changes and returns out to an excel file named returns
    """
    client = EodHistoricalData(key)
    temp = pd.DataFrame()
    ...
    print(f"Data retrived and saved to returns.xlxs in {os.getcwd()}")
...
def screen_example(symbols,key):
    ...
    for ticker in symbols:
        try:
            ...
            print(f"{ticker} not avaliable skipping")
...
def main():
    key = open('api.txt').read()
    #print(get_security_type(get_exchange_data(key)))
    #energy = get_sp(symbols=True, sector ="Information Technology")
    #get_data(*energy,key= key,path= 'mine2')
    #get_closing_prices(folder = 'mine')
    #print(returns_from_closes('mine', '0-closes.csv'))
    #print(get_corr(returns_from_closes('mine', '0-closes.csv')))
